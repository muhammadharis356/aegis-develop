...
async function cloneRepository(
  ...
  const tempDir = path.join(os.tmpdir(), `aegis-ai-scan-${Date.now()}-${Math.random().toString(36).substring(7)}`);
...
export async function exportScanAsAIBOM(
  ...
  return {
    ...
    metadata: {
      ...
      tools: [
        { name: "Aegis.AI AI Detection", version: "1.0.0" },
      ],
      repository: {
