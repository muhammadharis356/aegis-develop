...
check_and_download_compose_files() {
    # Check if we have the local files, otherwise warn user
    if [ ! -f "docker-compose.yml" ]; then
        echo "Error: docker-compose.yml not found. Please ensure you are running this script from the root of the cloned repository."
        exit 1
    fi

    if [ ! -f "docker-compose.prod.yml" ]; then
        echo "Warning: docker-compose.prod.yml not found. Setup might be incomplete."
    fi
}
